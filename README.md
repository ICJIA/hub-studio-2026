...
![ICJIA Research Hub Studio — write in plain language, preview exactly as published, and ship with built-in editorial review. A document with a highlighted passage and its review comment, over the Draft → Review → Publish pipeline.](public/og-image.png)
...
- **Security:** independently red/blue-team audited **four** times (production, the public demo, the demo-roles/main-files/tour/dependency surface, and the annotations/preview/card-view surface) — **0 critical issues**; in-repo fixes done and covered by **946 automated tests** ([`docs/security-audit.md`](docs/security-audit.md)).
...
**Between passes,** every security-relevant change is recorded in the report's **post-audit delta log** (newest entry 2026-07-17: the media-library picker surface — flagged for the next adversarial pass — the unsaved-work localStorage guard, the pre-merge live-filter wire-format fix, the CI-guarded staging-host override, dependency currency incl. dompurify 3.4.12, and the social-card meta). A fresh `npm audit` on 2026-07-17 reconfirms **0 critical / 0 high / 0 moderate** (1 known dev-only Low, esbuild on Windows, unchanged).
...
**Current totals:** **946 tests** across **114 test files** (73 unit + 41 Nuxt component).
...
```bash
npm test            # run all 946 tests once (vitest run)
