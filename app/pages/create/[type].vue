...
// "Add sample article" links here with ?sample=1 — seed the form once with a realistic draft so the
// author can preview/edit and then Save (the Save is the create that actually writes to Strapi).
// Built once (not a computed): buildSampleArticle() is randomized per call.
const sampleArticle = type === 'article' && route.query?.sample === '1' ? buildSampleArticle() : undefined
</script>
<template>
  <div>
