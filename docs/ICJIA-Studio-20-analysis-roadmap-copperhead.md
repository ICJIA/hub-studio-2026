...
| 10 | **✅ Scheduled (v0.8.3, 2026-07-17).** **Accessibility polish backlog** (four small riders from the annotations review: color-swatch radiogroup semantics, roving toolbar tabindex, drawer dialog semantics, document-level keyboard-create listener). Now `ROADMAP.md` Next №1 — a single small release. | The app measures AA-clean; these are screen-reader/keyboard *refinements* already on the tracked list — scheduled so they don't evaporate. | ROADMAP Next №1; runbook §6 pointer | ~1 day |
