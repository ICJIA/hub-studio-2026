...
| # | Recommendation | Why it matters | Evidence in repo | Effort |
|---|---|---|---|---|
| 7 | **◑ Uptime half done (v0.8.2, 2026-07-17); error-reporter half deferred by decision (2026-07-17).** **Error monitoring + uptime checks.** Uptime: UptimeRobot probes on the Studio URL and the Strapi `/_health` endpoint (returns 204 unauthenticated — verified) are runbook'd (§4a) and turn on at cutover. Error reporter (Sentry): management chose to build it in at a later date; the design notes for the future build-in are recorded in `ROADMAP.md` (Deferred). | After launch, the only error signal today is "an author emails someone." A tool this central to the agency's most-read content deserves to know it's broken before its users do. | Runbook §4a; ROADMAP Deferred entry | uptime: done · reporter: deferred |
...
| 10 | **✅ Done (v0.8.4, 2026-07-17).** **Accessibility polish backlog** — all four riders from the annotations review shipped test-first as one small release: color swatches are a true **radiogroup** (`role="radio"` + `aria-checked`); the reviewer toolbar is **one tab stop** (roving tabindex, ARIA APG toolbar pattern); the mobile comments drawer is a labelled **modal dialog** (focus in, Tab wrap, Escape closes + restores); keyboard-create is a **hard-scoped document-level listener** (now fires where keyboard selections really leave focus; never from links/buttons/inputs — an in-article-link over-capture was fixed in the same pass). | The app measured AA-clean before; these deepen the screen-reader/keyboard experience — and the doc-level listener fixed two real keyboard defects found while building it. | v0.8.4 · 11 new tests (879/109) | ~1 day (actual) |
